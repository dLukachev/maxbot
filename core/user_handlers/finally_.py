...
async def make_target_is_done_finally(
    ...
    try:
        try:
            ...
            await update_menu(
                context,
                callback.message,
                text="Выбери что ты выполнил(а) f:",
                attachments=[
                    inline_keyboard_from_items_with_checks_finally(
                        model_groups, initial_checked, "finally_done"
                    )
                ],
            )  # type: ignore
    except Exception:
...
async def take_id_and_change_finally_isdone(
    callback: MessageCallback, context: MemoryContext
):
    user_state = await context.get_state()
    if user_state == "UserStates:counted_time":
...
async def commit_done_handler_finally(
    callback: MessageCallback, context: MemoryContext
):
    data = await context.get_data() or {}
    pending = set(data.get("pending_done", []))
    ...
    try:
        await callback.message.edit(
            "Теперь пора ставить новые цели, жми кнопку как будешь готов",
...
async def cancel_change_finally_target(
    ...
    try:
        ...
        await callback.message.answer(
            'Если ничего не забыл отметить, то жми "готово"!',
