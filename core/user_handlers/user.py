...
NEED_WRIGHTING_TARGET = (
    "Необходимо поставить цели для дальнейшей работы с ботом. Скорее жми на кнопку!"
)
...
async def blocker_callback(callback: MessageCallback, context: MemoryContext):
    ...
    try:
        await callback.message.edit(
            text=NEED_WRIGHTING_TARGET, attachments=[create_new_target_kb]
        )
    except:
...
async def handle_dialog_cleared(event: DialogCleared, context: MemoryContext):
    ...
    await event.bot.send_message(
        ...
        text="Для получения подробной информации о работе бота введите /help",
    )
    if not check:
        await UserCRUD.create(
            tid=event.from_user.user_id,
...
async def handle_bot_started(event: BotStarted, context: MemoryContext):
    ...
    if not check:
        ...
        await event.bot.send_message(
            chat_id=event.chat_id,
            user_id=event.user.user_id,
            text="Вот здесь нажми на кнопку и следуй дальнейшим инструкциям",
            attachments=[create_new_target_kb],
        )  # type: ignore
        return
...
async def get_and_wright_targets_in_db(message: MessageCreated, context: MemoryContext):
    ...
    await message.message.answer(
        f"Твой список:\n{answer}\nВерно?",
        attachments=[confirmation if not is_finally else confirmation_finally],
    )
    (
        await context.set_data({"targets": texts})
        if not is_finally
        else await context.set_data({"targets": texts, "finally": True})
    )
...
async def delete_target(callback: MessageCallback, context: MemoryContext):
    ...
    for t in items:
        row = [
            Item(
                id=t.id, description=t.description, is_done=getattr(t, "is_done", False)
            )
        ]
        model_groups.append(row)
...
async def start_session_choose_target(message: MessageCallback, context: MemoryContext):
    ...
    if not targets:
        await update_menu(
            context,
            message.message,
            text="Для этого вам нужны цели, нажмите на кнопку и поставите их прямо сейчас!",
            attachments=[create_new_target_kb],
        )
        return
...
@user.message_callback(
    F.callback.payload.startswith("start_target:"),
    UserStates.choosing_target_for_session,
)
@look_if_not_target
async def start_going(callback: MessageCallback, context: MemoryContext):
    ...
    if not target_id_str.isdigit():
        await update_menu(
            context, callback.message, text=ERROR_TEXT, attachments=[start_kb]
        )
        await context.clear()
        return
    ...
    if session:
        await update_menu(
            context,
            callback.message,
            text="У вас уже есть открытая сессия, ее необходимо завершить, прежде чем начинать новую",
            attachments=[stop_kb],
        )
    ...
    await update_menu(
        context,
        callback.message,
        text=f"Фиксирую старт: {now.strftime('%H:%M:%S')}",
        attachments=[stop_kb],
    )
...
async def stop_going(message: MessageCallback, context: MemoryContext):
    ...
    if not session:
        await update_menu(
            context, message.message, text=ERROR_TEXT, attachments=[start_kb]
        )
        return
    ...
    await update_menu(
        context,
        message.message,
        text=f"Сессия завершена. Добавлено: `{elapsed_str}`",
        attachments=[start_kb],
    )


async def draw_profile(
    message: MessageCallback | MessageCreated, context: MemoryContext
):
    ...
    if str(user_state) == "UserStates:draw_new_prifile":
        ...
        await update_menu(
            context, message.message, text=answer, attachments=[profile_kb]
        )
...
async def adjust_target_time_start(callback: MessageCallback, context: MemoryContext):
    ...
    await update_menu(
        context, callback.message, text=prompt, attachments=[back_to_profile_kb]
    )
...
async def adjust_target_time_finish(message: MessageCreated, context: MemoryContext):
    ...
    if seconds is None:
        await message.message.answer(
            "Неверный формат времени. Попробуйте еще раз (чч:мм:сс)."
        )
        return
    ...
    if not user_updated:
        await update_menu(
            context,
            message.message,
            text="Ошибка! Не удалось обновить профиль.",
            attachments=[start_kb],
        )
        await context.clear()
        return
...
async def get_targets(message: MessageCreated, context: MemoryContext):
    ...
    if not target:
        await update_menu(
            context,
            message.message,
            text="Почему то не вижу Ваших целей на сегодня( Нажмите кнопку и поставьте их!",
            attachments=[create_new_target_kb],
        )
        await context.set_state(UserStates.wrighting_targets)
        return
