...
def look_if_not_target(
    ...
    async def wrapper(*args, **kwargs):
        try:
            ...
            if (
                str(user_state) == "UserStates:counted_time"
                and event.callback.payload != "stop_session"
                if not mc
                else event.message.payload != "stop_session"
            ):
            ...
            user, targets = await TargetCRUD.get_all_target_today(
                user_id, datetime.today()
            )
